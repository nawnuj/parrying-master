...
플레이어 공격 몽타주에 `Attack1`, `Attack2`, `Attack3` Section을 구성하여 세 종류의 공격 애니메이션이 연결되는 3단 콤보를 구현했다. 각 Section은 기본적으로 독립되어 있으며, 입력 가능 구간에 다음 공격이 예약된 경우에만 다음 Section으로 연결된다.

`ComboWindowOpen`과 `ComboWindowClose` Montage Notify를 이용해 다음 공격 입력이 허용되는 구간을 제어한다. 공격 키를 빠르게 반복해도 입력 가능 구간 밖의 입력은 무시하며, 예약 입력은 한 번만 저장된다.

공격 몽타주의 정상 종료와 피격 또는 사망으로 인한 강제 중단에서 콤보 단계, 입력 가능 상태 및 예약 입력을 초기화하도록 구성했다. 각 공격의 `AttackHit` Notify에서는 기존 근접 공격 판정과 피해량을 그대로 재사용한다.

발차기를 사용하는 3타에서 Foot IK로 인해 다리가 꼬이는 문제는 Animation Blueprint의 노드 순서를 `Main States → Control Rig → DefaultSlot → Output Pose`로 변경하여 해결했다.

다음 단계에서는 공격 중 방향 전환과 짧은 전진 이동을 보완하고, 플레이어 사망 애니메이션과 레벨 재시작 기능을 구현할 예정이다.
