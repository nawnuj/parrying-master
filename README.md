...
| 이름 | 역할 |
|---|---|
| `APMCharacter` | 이동, 카메라, 회피, 공격 시작 방향 보정, 3단 콤보, 공격 판정, 피격 상태, 경직, 타이밍 및 방향 기반 패링, 패링 성공 소비, 반격 가능 구간 및 강화 반격 피해 처리 |
...
현재는 첫 번째 공격을 시작할 때만 방향을 명시적으로 보정한다. 콤보 중에는 Character Movement의 기본 회전이 적용될 수 있지만, Attack2와 Attack3 전환 시점의 별도 방향 보정과 공격 전진 이동은 아직 구현하지 않았다.
