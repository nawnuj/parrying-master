...
패링, 반격, 일반 피격과 적 공격의 시간 및 피해량을 함께 점검하여 전투 밸런스의 1차 기준값을 확정했다.

반격 가능 시간과 적의 패링 경직 시간이 모두 1초여서 늦게 반격을 입력하면 공격이 적중하기 전에 적의 경직이 풀릴 수 있었다. 이를 보완하기 위해 `BP_EnemyCharacter`의 패링 경직 시간을 1초에서 1.3초로 늘렸다.

패링 판정 0.2초, 패링 쿨다운 0.6초, 반격 가능 시간 1초, 반격 피해 40과 적 공격 피해 15 및 공격 쿨다운 1.5초는 기존 값을 유지했다. 테스트를 통해 반격 UI가 거의 끝나는 시점에 공격해도 적의 경직 안에서 반격이 자연스럽게 연결되는 것을 확인했다.
