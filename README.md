...
| 이름 | 역할 |
|---|---|
| `APMCharacter` | 이동, 카메라, 방향 기반 회피 몽타주와 시간 기반 이동, 회피 무적 판정, 공격 및 콤보 단계별 방향 보정과 전진 이동, 3단 콤보, 공격 판정, 피해 차단, 피격 상태, 경직, 타이밍 및 방향 기반 패링, 패링 성공 소비, 반격 가능 구간 및 강화 반격 피해 처리 |
