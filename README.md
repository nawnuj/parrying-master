...
| 이름 | 역할 |
|---|---|
| `APMCharacter` | 이동, 카메라, 방향 기반 회피 몽타주와 시간 기반 이동, 회피 무적 판정, 공격 및 콤보 단계별 방향 보정과 전진 이동, 3단 콤보, 공격 판정, 피해 차단, 피격 상태, 경직, 타이밍 및 방향 기반 패링, 반격 처리, 사망 몽타주와 지연된 레벨 재시작 관리 |
...
회피 전용 Animation Sequence `A_Player_Dodge`와 Animation Montage `AM_Player_Dodge`를 추가했다. `A_Player_Dodge`에서는 Root Motion을 비활성화하고 Root Lock을 적용하여 애니메이션이 제자리 회피 자세만 담당하도록 구성했다.

회피 입력 시 마지막 이동 입력 방향을 확인하고, 입력 방향이 있으면 해당 방향으로 캐릭터를 회전시킨 뒤 회피한다. 이동 입력이 없으면 현재 캐릭터가 바라보는 방향을 회피 방향으로 사용한다.
...
플레이어 사망 후 일정 시간이 지나면 현재 레벨을 다시 시작하도록 확장했다. `DeathRestartDelay`의 기본값은 3초이며, `DeathRestartTimer`가 재시작 시점을 관리한다.

재시작 시 `UGameplayStatics::GetCurrentLevelName()`으로 현재 레벨 이름을 구하고 `UGameplayStatics::OpenLevel()`로 같은 레벨을 다시 연다. 레벨 전체를 다시 불러오므로 플레이어의 체력과 입력뿐만 아니라 적의 위치, 체력과 AI 상태도 함께 초기화된다.

통합 테스트 과정에서 적이 일정 거리에서 이동을 멈춘 채 공격하지 않는 거리 데드존을 발견했다. 플레이어가 바라보는 방향은 공격 조건에 사용되지 않았으며, AI Controller와 Enemy Character가 각각 `AcceptanceRadius`와 `EnemyAttackRange`를 별도의 공격 기준으로 사용하던 구조가 원인이었다.

`APMEnemyCharacter::GetAttackRange()`를 추가하고 AI Controller도 Enemy Character가 소유한 공격 거리를 사용하도록 변경했다. 이제 `EnemyAttackRange`는 공격 시작 판단을 담당하고 `AcceptanceRadius`는 NavMesh 이동의 도착 허용 오차로만 사용한다.
