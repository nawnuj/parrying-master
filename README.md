...
| `APMEnemyCharacter` | 적 캐릭터의 체력, 공격 몽타주, 근접 공격 판정, 패링 가능 구간, 패링 성공 및 피해 무효화 판정, 전투 상태 정리 및 사망 처리 |
...
현재 패링 성공 시 피해만 무효화되며 플레이어의 패링 판정은 기존 타이머가 끝날 때까지 유지된다. 적도 공격 몽타주를 끝까지 재생하므로 다음 단계에서 패링 성공 소비, 적 공격 중단 및 경직을 구현할 예정이다.
